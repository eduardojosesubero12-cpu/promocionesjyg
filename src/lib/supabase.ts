...
export function dbToRows(db: CRMData): Record<string, any[]> {
  return {
    ...
    historial_tasas: db.historialTasas.map((x) => ({ id: x.id, fecha: x.fecha, usd: x.usd, euro: x.euro, paralelo: x.paralelo, fuente: x.fuente, actualizado: x.actualizado })),
...
export function rowsToDb(rows: Record<string, any[]>, base: CRMData): CRMData {
  const estudiantes: Estudiante[] = (rows.estudiantes || []).map((r) => {
    const pagos: Pago[] = (rows.pagos || []).filter((p) => p.estudiante_id === r.id).map((p) => ({ id: p.id, fecha: p.fecha, monto: Number(p.monto), metodo: p.metodo, bs: !!p.bs, tasa: Number(p.tasa), usd: Number(p.usd), referencia: p.referencia || "", observacion: p.observacion || "" }));
    const adicionales: AdicionalItem[] = (rows.adicionales_items || []).filter((a) => a.estudiante_id === r.id).map((a) => ({ producto: a.producto, cantidad: Number(a.cantidad), precio: Number(a.precio), talla: a.talla || "" }));
    return { id: r.id, pedido: r.pedido, nombre: r.nombre, telefono: r.telefono || "", representante: r.representante || "", ci: r.ci || "", escuelaId: r.escuela_id || "", docenteId: r.docente_id || "", grado: r.grado || "Bachiller", seccion: r.seccion || "A", paqueteId: r.paquete_id || "premium", precioPaquete: Number(r.precio_paquete) || 0, adicionales, pagos, estadoPedido: r.estado_pedido || "Registrado", fechaRegistro: r.fecha_registro || "", fechaEntrega: r.fecha_entrega || "", observaciones: r.observaciones || "", codigos: { ...codigosVacios(), ...(r.codigos || {}) } };
  });
  const cotizaciones: Cotizacion[] = (rows.cotizaciones || []).map((c) => ({ id: c.id, numero: c.numero, fecha: c.fecha || "", cliente: c.cliente || "", telefono: c.telefono || "", escuela: c.escuela || "", paqueteId: c.paquete_id || "premium", estado: c.estado || "Pendiente", nota: c.nota || "", adicionales: (rows.cotizacion_items || []).filter((i) => i.cotizacion_id === c.id).map((i) => ({ producto: i.producto, cantidad: Number(i.cantidad), precio: Number(i.precio), talla: i.talla || "" })) }));
  ...
  return {
    ...
    historialTasas: (rows.historial_tasas || []).map((x) => ({ id: x.id, fecha: x.fecha, usd: Number(x.usd), euro: Number(x.euro), paralelo: Number(x.paralelo), fuente: x.fuente || "dolarapi", actualizado: Number(x.actualizado) || 0 })),
...
export async function subirTodo(client: SupabaseClient, db: CRMData, onTabla?: (tabla: string, estado: "busy" | "ok" | "err", filas?: number) => void): Promise<void> {
...
export async function descargarTodo(client: SupabaseClient): Promise<Record<string, any[]>> {
  ...
  if (rows.estudiantes.length + rows.escuelas.length + rows.configuracion.length === 0)
