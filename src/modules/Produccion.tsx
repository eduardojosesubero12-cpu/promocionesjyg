                  ...
                  <tbody>
                    {Object.entries(prod.adicionales).map(([nombre, info]) => (
                      <tr key={nombre}>
                        ...
                        <td>{Object.keys(info.tallas).length ? <div className="flex flex-wrap gap-1.5">{Object.entries(info.tallas).map(([t, n]) => <Badge key={t} tone="blue">{t}: {n}</Badge>)}</div> : <span style={{ color: "var(--ink-faint)" }}>—</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
