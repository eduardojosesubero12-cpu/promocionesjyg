...
import { ArrowRight, Check, GraduationCap, Pencil, Plus, Receipt, Search, Smartphone, Trash2, Wallet } from "lucide-react";
    ...
    <div className="page">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        {ESTADOS_PEDIDO.map((s, i) => {
          ...
          const color = tone === "green" ? "var(--green)" : tone === "slate" ? "var(--slate)" : "var(--blue)";
          return (
            <button key={s} onClick={() => setFEstado(fEstado === s ? "" : s)} className="card p-3.5 text-left cursor-pointer transition-all hover:-translate-y-1 reveal" style={{ animationDelay: `${i * 50}ms`, outline: fEstado === s ? "2px solid var(--blue)" : "none" }}>
              <div className="font-display font-bold text-[21px]" style={{ color }}>{n}</div>
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="tbl">
            <thead><tr><th>N° Pedido</th><th>Estudiante</th><th>Registro</th><th>Paquete</th><th>Total</th><th>Saldo (Bs)</th><th>Estado</th><th></th></tr></thead>
            <tbody>
              {lista.map((e) => {
                ...
                return (
                  <tr key={e.id} className="cursor-pointer" onClick={() => setRoute("estudiantes", { open: e.id })}>
                    ...
                    <td><div className="font-display font-semibold text-[13.5px]">{e.nombre}</div><div className="text-[11px]" style={{ color: "var(--ink-faint)" }}>{e.grado} “{e.seccion}”</div></td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {db.cotizaciones.map((c, i) => (
          <div key={c.id} className="card p-5 reveal transition-all hover:-translate-y-1 hover:shadow-[var(--shadow-md)]" style={{ animationDelay: `${Math.min(i, 8) * 55}ms` }}>
            ...
            {c.adicionales.length > 0 && <p className="text-[12px] mt-1 mb-0" style={{ color: "var(--ink-soft)" }}>Adicionales: {c.adicionales.map((a) => `${a.cantidad}× ${a.producto}`).join(", ")}</p>}
            ...
            <div className="flex gap-2 mt-4 flex-wrap">
              ...
              {c.telefono && (
                <a className="btn btn-soft btn-sm" style={{ background: "var(--green-tint)", color: "#1f9d55" }} href={waLink(c.telefono, `Hola ${c.cliente}, le saluda Promociones JyG 🎓. Su cotización ${c.numero} por ${fmtUSD(total(c))} sigue disponible.`)} target="_blank" rel="noreferrer">
