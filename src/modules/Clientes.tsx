                ...
                <tbody>
                  {representantes.map((r) => (
                    <tr key={r.nombre + r.telefono}>
                      ...
                      <td><div className="flex flex-col gap-0.5">{r.hijos.map((x) => <span key={x.id} className="text-[12.5px]">{x.nombre} <span style={{ color: "var(--ink-faint)" }}>· {x.pedido}</span></span>)}</div></td>
                      <td><div className="flex flex-wrap gap-1">{r.hijos.map((x) => <Badge key={x.id} tone={x.paqueteId === "lujo" ? "gold" : x.paqueteId === "premium" ? "blue" : "slate"}>{PAQUETES[x.paqueteId].nombre}</Badge>)}</div></td>
