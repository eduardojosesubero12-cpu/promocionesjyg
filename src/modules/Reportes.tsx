...
export default function Reportes() {
  ...
  const porGrado = useMemo(() => GRADOS.map((g) => ({ grado: g, n: filtrados.filter((e) => e.grado === g).length })).filter((x) => x.n > 0), [filtrados, db.estudiantes]);
  ...
  const exportFinanzas = () => {
    downloadFile(`reporte-finanzas-${todayISO()}.csv`, toCSV(
      ...
      filtrados.map((e) => {
        const t = estudianteTotales(e);
        return [e.pedido, e.nombre, db.escuelas.find((x) => x.id === e.escuelaId)?.nombre || "", `${e.grado} ${e.seccion}`, PAQUETES[e.paqueteId].nombre, t.total.toFixed(2), t.abonado.toFixed(2), t.saldo.toFixed(2), t.estadoPago, e.estadoPedido];
      })
    ));
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={x.l} className="card p-5 reveal" style={{ animationDelay: `${i * 60}ms` }}>
            <div className="flex items-center gap-2 text-[11px] font-display font-semibold uppercase tracking-wider" style={{ color: "var(--ink-faint)" }}>
              {i === 0 ? <Wallet size={13} /> : i === 1 ? <Wallet size={13} /> : <Wallet size={13} />} {x.l}
            </div>
        ...
        <div className="card p-5">
          ...
          <div className="flex flex-col gap-2">
            {Object.entries(prod.materiales).filter(([, q]) => q > 0).length === 0 && (
              <p className="text-[13px] m-0 py-3" style={{ color: "var(--ink-faint)" }}>Sin materiales pendientes con los filtros actuales.</p>
            )}
